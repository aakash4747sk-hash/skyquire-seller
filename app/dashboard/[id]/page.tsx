    ...
    <div>
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap mb-5">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">{sub.sector}{sub.location ? ` · ${sub.location}` : ""}{sub.asking_price ? ` · Asking ${sub.asking_price}` : ""}</p>
        </div>
            ...
            {STEPS.map((s, i) => (
              <div key={s} className="flex items-center flex-1 last:flex-none">
                <div className="flex flex-col items-center">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold ${i <= currentStep ? "bg-brand text-white" : "bg-white/10 text-slate-400"}`}>
                    {i < currentStep ? "✓" : i + 1}
                  </div>
                  <span className={`text-[11px] mt-1.5 ${i <= currentStep ? "text-slate-200 font-medium" : "text-slate-400"}`}>{STEP_LABEL[s]}</span>
                </div>
                {i < STEPS.length - 1 && <div className={`flex-1 h-0.5 mx-2 ${i < currentStep ? "bg-brand" : "bg-white/10"}`} />}
              </div>
            ))}
          </div>
        </div>
      ...
      {sub.review_notes && (
        <div className="card p-5 mb-4" style={{ background: meta.bg, borderColor: "transparent" }}>
          ...
          <p className="text-sm text-slate-200 whitespace-pre-wrap">{sub.review_notes}</p>
        </div>
      )}
      ...
      {sub.status === "needs_info" && (
        <div className="card p-5 mb-4">
          <p className="font-semibold text-sm text-slate-200 mb-1">We need a bit more</p>
          <p className="text-xs text-slate-400 mb-3">Upload the requested documents and your submission goes back into review automatically.</p>
      ...
      {/* Details */}
      <div className="card p-5 mb-4">
        <p className="font-semibold text-sm text-slate-200 mb-3">Details</p>
        {sub.description && <p className="text-sm text-slate-300 mb-4">{sub.description}</p>}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 text-sm">
          {[
            ...
            <div key={k}><p className="text-xs text-slate-400">{k}</p><p className="font-medium text-slate-200">{v}</p></div>
          ))}
        </div>
      </div>
      ...
      <div className="card p-5">
        <p className="font-semibold text-sm text-slate-200 mb-3">Documents</p>
        {docs.length === 0 ? (
          <p className="text-sm text-slate-400">No documents uploaded.</p>
        ) : (
          <div className="space-y-2">
            {docs.map((d) => (
              <div key={d.id} className="flex items-center gap-3 p-2.5 rounded-xl border border-white/10">
                <span>📄</span>
