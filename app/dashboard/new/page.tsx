...
export default function NewSubmission() {
  ...
  return (
    <div>
      <button onClick={() => router.push("/dashboard")} className="text-sm text-slate-400 hover:text-slate-200 mb-4">← Back</button>
      <h1 className="font-display text-2xl font-bold mb-1">Submit your business</h1>
      <p className="text-slate-400 text-sm mb-6">Tell us about your business. Our team will verify it before it goes live to buyers.</p>

      {error && <div className="mb-4 p-3 rounded-xl bg-red-50 border border-red-200 text-red-600 text-sm">{error}</div>}

      <form onSubmit={submit} className="space-y-5">
        <div className="card p-5 space-y-4">
          <p className="font-semibold text-sm text-slate-200">The business</p>
          <div>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">What does the business do?</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Highlights (comma-separated)</label>
        ...
        <div className="card p-5 space-y-4">
          <p className="font-semibold text-sm text-slate-200">Financials</p>
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Asking price (₹ Cr) *</label>
            ...
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Annual revenue (₹ Cr)</label>
              <input type="number" step="0.1" value={form.revenueCr} onChange={(e) => set("revenueCr", e.target.value)} className="input" placeholder="3" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">EBITDA margin</label>
              <input value={form.ebitda} onChange={(e) => set("ebitda", e.target.value)} className="input" placeholder="e.g. 18%" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Employees</label>
              <input type="number" value={form.employees} onChange={(e) => set("employees", e.target.value)} className="input" placeholder="25" />
            </div>
            <div>
              <label className="block text-xs font-medium text-slate-300 mb-1.5">Founded (year)</label>
          ...
          <div>
            <label className="block text-xs font-medium text-slate-300 mb-1.5">Reason for selling</label>
        ...
        <div className="card p-5 space-y-3">
          <p className="font-semibold text-sm text-slate-200">Documents</p>
          <p className="text-xs text-slate-400">Upload financials, GST returns, or incorporation docs — these stay private and are only seen by our verification team.</p>
          <input type="file" multiple onChange={(e) => setFiles(Array.from(e.target.files || []))} className="text-sm" />
          {files.length > 0 && <p className="text-xs text-slate-400">{files.length} file{files.length > 1 ? "s" : ""} selected</p>}
        </div>

        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => router.push("/dashboard")} className="px-5 py-2.5 rounded-xl border border-white/10 text-sm text-slate-300 hover:bg-white/5">Cancel</button>
